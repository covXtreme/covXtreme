classdef DeltaLaplaceExpected<HeffernanTawn
    ...
    properties
        %SigmaTransform - if variance should be a sqrt
        %                - if standard deviation one to one mapping
        SigmaTransform=@(x) x;
                ...
                switch iP
                    case 1 %alpha and mu
                        ...
                        for iDmn=1:HT.nDmn-1 %loop iDmn
                            ...
                            E11 = diag(accumarray(A(:,1),t1(:,iDmn),[HT.nPrm(1),1],@sum,0)+L.*P);
                            if  HT.TwoParamFlag
                                H(:,:,iDmn)=E11;
                            else
                                ...
                                E33=diag(accumarray(A(:,3),t1(:,iDmn),[HT.nPrm(3),1],@sum,0)+L.*P);
                                ...
                                if HT.NonStat(1) &&  HT.NonStat(3) %both non stationary
                                    E13 = diag(accumarray(A(:,1),t1(:,iDmn),[HT.nPrm(1),1],@sum,0)); % nBin  x nBin
                                elseif HT.NonStat(1)
                                    E13 = accumarray(A(:,1),t1(:,iDmn),[HT.nPrm(1),1],@sum,0); % nBin  x 1
                                elseif HT.NonStat(3)
                                    E13 = accumarray(A(:,3),t1(:,iDmn),[HT.nPrm(3),1],@sum,0)'; % 1 x nBin
                    ...
                    case 2 %beta and sigma
                        ...
                        for iDmn=1:HT.nDmn-1 %loop iDmn
                            ...
                            E22 = diag((accumarray(A(:,2),(t1(:,iDmn)),[HT.nPrm(2),1],@sum,0))+L.*P);
                            ...
                            if  HT.TwoParamFlag
                                ...
                                E44=diag((accumarray(A(:,4),(t1(:,iDmn)),[HT.nPrm(4),1],@sum,0))+L.*P);
