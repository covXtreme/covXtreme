classdef HeffernanTawn
    ...
    properties        
        ...
        Alg= 'fit_newtonraphson';  %algorithm used to fit HT model either fit_fminsearch or fit_newtonraphson
    ...
    methods
        ...
        function Sml=SimulateIS(HT,Mrg,nRls)
            ...
            Sml.logf=NaN(nRls,HT.nDmn);  %%log importance weights on original margins
            logg_Stn=NaN(nRls,HT.nDmn);  %log importance weights on standard margins
            Sml.logg=NaN(nRls,HT.nDmn);  %%log importance weights on original margins                       
            ...
            for iDmn=1:HT.nDmn
                ...
                Sml.logg(:,iDmn)=-log(Rng); %importance weights                    
                ...
                logg_Stn(:,iDmn)=Sml.logg(:,iDmn)-J_Unf_GamGP+J_Unf_Stn;
                ...
                Sml.logf(:,iDmn)=logf_Stn(:,iDmn)+J_Unf_GamGP-J_Unf_Stn; 
                               
            end %loop over dimensions
            
            Sml.logfog=Sml.logf-Sml.logg;
        ...
        function HT=ConditionalReturnValue(HT,Mrg,nRls)
            ...
            switch HT.RVMth
                ...
                case 2 %Fixed value from cuser input
                    ...
                    RV.X=permute(HT.RVFix,[1,3,2]);%gives you a nBin x 1 x nRtrPrd
                    RV.X=repmat(RV.X,1,nRls,1);%need to check whether this is necessary
            ...
            for iRtr=1:HT.nRtr %loop over return periods
                ...
                switch HT.RVMth
                    case 1
                     ...
                     case 2
                        %P=CDF(RV.X(:,:,iRtr),[1,2,3]) %transform form uniform to standard margins using CDF
                        P=Mrg(1).CDF(RV.X(:,1,iRtr),(1:HT.nBin)',I); %transform form uniform to standard margins using CDF
                end
                RV.X_Stn(:,:,iRtr)=INV_Standard(Mrg(1),P); %transform form uniform to standard margins using CDF
    ...
    methods% (Access = private)
        function HT=Fit_private(HT,IExc,iBt)
            ...
            HT.Sig(:,:,iBt)=sqrt(tPrm(HT.PrmInd==4,:)); %TODO if statement sd or variance 
        ...
        function p=fit_newtonraphson(HT,X,Y,A,Lam)
            ...
            %Lam  %smoothness parameter
            b0=0;
            p=HT.startingSolution(X,Y,A,b0);
            %testing Gradient of the model
            [~,H1]=HT.Gradient(1,X,Y,A,p,Lam);
            [~,H2]=HT.Gradient(2,X,Y,A,p,Lam);
            cnt=0;
            if min(eig(H1))<0 || min(eig(H2))<0
                while min(eig(H1))<0 || min(eig(H2))<0
                    cnt=cnt+1;
                    b0=b0-0.5;
                    p=HT.startingSolution(X,Y,A,b0);
                    [~,H1]=HT.Gradient(1,X,Y,A,p,Lam);
            ...
            for iIt=1:maxIter %iteration loop
                ...
                for iP=1:2
                    ...
                    if min(eig(H))<0 %trying to deal with a non positive semi-definite covariance matrix
                        continue
                    end    
                    newp=p;
                    for iDmn=1:HT.nDmn-1
        ...
        function p0=startingSolution(HT,X,Y,A,b0)
           ...
           if nargin<=4
            ...
            %indexing over the different directional bins    
                for i = 1:length(b0)
                    diffY=(Y-Q*p);
                    R(B(:,i)==1)=diffY(B(:,i)==1)./X(B(:,i)==1).^b0(i);
                end
            else
            %stationary in b     
                R=(Y-Q*p)./(X.^b0);   
            ...
            if HT.NonStat(4)%TODO for either the variance and std
                v0=nan(HT.nPrm(4),1);
                for i = 1:length(v0)
                    v0(i)=std(R(B(:,i)==1)).*0.1;%.*0.1;%0.1;%
                end
            else
                v0=(ones(HT.nPrm(4),1).*std(R));
            end
        ...
        function PLOGL=likelihood(HT,X,Y,A,p,L)
            ...
            MeanTerm=bsxfun(@times,Alp_Dat,X)+bsxfun(@times,M_Dat,Xb);
            StdTerm=bsxfun(@times,sqrt(V_Dat),Xb);
            R=abs(Y-MeanTerm)./(Kappa.*StdTerm);
            %NLOGL=sum(sum(0.5*((Y-bsxfun(@times,Alp_Dat,X)-bsxfun(@times,M_Dat,Xb)).^2./(Std).^2)+log(Std)));
            NLOGL=sum(HT.n.*(-log(HT.Delta)+log(2)+gammaln(1./HT.Delta))+ sum((abs(Y-bsxfun(@times,Alp_Dat,X)-bsxfun(@times,M_Dat,Xb))./(Kappa.*StdTerm)).^HT.Delta+log(Kappa.*StdTerm)));
            if HT.nBin>1
        ...
        function [G,H]=Gradient(HT,iP,X,Y,A,p,L)
            ...
            StdTerm=Kappa.*(Xb).*sqrt(V_Dat);%standard deviation of the distribtuion
            R=Y-MeanTerm; 
            ...
            switch iP
                case 1 %alpha and mu
                    ...
                    RsdAlp=(Alp-mean(Alp,1));%residual alpha difference (penalty term)
                    for iDmn=1:HT.nDmn-1
                    ...
                    if HT.TwoParamFlag
						...
						RsdM=(M-mean(M,1)); %residual mu difference (penalty term)
                        for iDmn=1:HT.nDmn-1
                ...
                case 2 %beta and sigma
					...
					RsdBet=(Bet-mean(Bet,1));%residual alpha difference (penalty term)
                    for iDmn=1:HT.nDmn-1
                    ...
                    if HT.TwoParamFlag
						...
						RsdV=(V-mean(V,1)); %residual V difference (penalty term)
                        for iDmn=1:HT.nDmn-1
            ...
            if nargout==2
                ...
                switch iP
                    case 1 %alpha and mu
                        ...
                        for iDmn=1:HT.nDmn-1 %loop iDmn
                            ...
                            if  HT.TwoParamFlag
                                ...
                                t1=1./(V_Dat.^2);
                                E33 =diag(accumarray(A(:,3),t1(:,iDmn),[HT.nPrm(3),1],@sum,0))+2.*L.*(HT.nPrm(3)-1).*(HT.nPrm(3)-2)./(HT.nPrm(3).^2);
                                %t1=(tDel-1).*tDel.*X.^(Bet_Dat+1).*abs(R).^(tDel-2)./((StdTerm).^tDel);
                                t1=(X.^(1-Bet_Dat))./(V_Dat.^2);
                    ...
                    case 2 %beta and sigma
						...
						%Y_m_aX=(Y-Alp_Dat.*X);
						%t1=((HT.Delta.*(log(X).^2).*Y_m_aX)./((StdTerm).^HT.Delta)).*...
						%		(HT.Delta.*(Y_m_aX)-Xb.*M_Dat).*abs(R).^(HT.Delta-2);
                        t1=(log(X).^2).*(2 + M_Dat.^2./V_Dat.^2);
                        for iDmn=1:HT.nDmn-1 %loop iDmn
                            ...
                            if  HT.TwoParamFlag
                                ...
                                %d2L/DLsigma2
								%t1=(1./(2.*V_Dat(:,iDmn).^2));
								%t1=(HT.Delta+1).*HT.Delta.*abs(R).^(HT.Delta)./(((StdTerm).^HT.Delta).*(S_Dat.^2))-1./(S_Dat.^2); 
                                t1=(2./(V_Dat(:,iDmn).^2));
                                E44 =diag((accumarray(A(:,4),(t1(:,iDmn)),[HT.nPrm(4),1],@sum,0))+2.*L.*(HT.nPrm(4)-1).*(HT.nPrm(4)-2)./(HT.nPrm(4).^2));
                                
                                 %d2L/DLbetasigma
                                %t1=(1./V_Dat).*log(X);
								%t1=zeros(size(X));%HT.Delta.^2.*log(X).*Y_m_aX.*R.*abs(R).^(HT.Delta-2)./((StdTerm).^(HT.Delta).*S_Dat);
                                t1=2.*log(X)./V_Dat;
                                if HT.NonStat(2) &&  HT.NonStat(4) %both non stationary
                                    E24 = diag(accumarray(A(:,2),(t1(:,iDmn)),[HT.nPrm(2),1],@sum,0)); % nBin  x nBin
                                elseif HT.NonStat(2)
                                    E24 = (accumarray(A(:,2),(t1(:,iDmn)),[HT.nPrm(2),1],@sum,0)); % nBin  x 1
                                elseif HT.NonStat(4)
                                    E24 = (accumarray(A(:,4),(t1(:,iDmn)),[HT.nPrm(4),1],@sum,0))'; % 1 x nBin
                                else %both stationary
                                    E24 = (sum(t1)); % 1 x 1
                                end
    ...
    methods(Static)
         ...
         function G=mygradfmincon(X,Y,A,p,L)
            ...
            S_Dat=sqrt(V);
            Xb=X.^Bet;  %X^b                       
            MeanTerm=Alp.*X+(Xb).*M; %mean term of the distribution
            StdTerm=Kappa*(Xb).*S.Dat;%standard deviation of the distribtuion
            R=Y-MeanTerm; 
			...
			t1=sum(-X.*HT.Delta.*R.*(abs(R).^(HT.Delta-2))./((StdTerm).^HT.Delta));
            for iDmn=1:nDmn-1
                G(1:nBin,iDmn) = accumarray(A,t1(:,iDmn),[nBin,1],@sum,0)+2.*L.*AlpRsd(:,iDmn);
            end            
            %dL/dbeta
			G(nBin+1,:)=sum(log(X)-HT.Delta.*R.*abs(R).^(HT.Delta-2).*M.*Xb.*log(X)./((StdTerm).^HT.Delta)...
                       -HT.Delta.*log(X).*abs(R).^(HT.Delta)./((StdTerm).^HT.Delta)); 
            %G(nBin+1,:) = sum((-1./V).*X.^(-2.*Bet).*log(X).*(Y - Al.*X).*(Y - Al.*X - X.^Bet.*M) + log(X));
            %dL/dmu
            G(nBin+2,:) = sum(-Xb.*HT.Delta.*R.*abs(R).^(HT.Delta-2)./((StdTerm).^HT.Delta));
			%sum((-1./V).*X.^(-Bet).*(Y - Al.*X - X.^Bet.*M));            
            %dL/dsigma            
            %G(nBin+3,:) = sum(-1./S.*(1./(2.*S).*X.^(-2*Bet).*(Y - Al.*X - X.^Bet.*M).^2 - 1/2));
            G(nBin+3,:) = sum((1./S_Dat)-(HT.Delta*abs(R).^(HT.Delta)./(S_Dat.*(StdTerm).^HT.Delta)));
			%(1./(2.*V)).*sum(1-(1./V).*((Y - Al.*X - X.^Bet.*M)./(X.^Bet)).^2);
