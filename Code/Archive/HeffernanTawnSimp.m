classdef HeffernanTawnSimp
    ...
    properties(Abstract)
        SigmaTransform;  %variance transfrom function
        VarStartSol;   %var or std in starting solution
    end
	
    methods (Abstract)
        [G,H]=Gradient(HT,iP,X,Y,A,p,L)       %Gradient function overloaded
        PLOGL=likelihood(HT,X,Y,A,p,L)        %Likelihood function overloaded
        
    end
    
    
    methods
        function HT=HeffernanTawnSimp(Mrg,HTNEP,NonStationary,CV,SmpLclRsdOn)
    ...
    methods% (Access = private)
        function HT=Fit_private(HT,IExc,iBt)
            ...
            HT.Sig(:,:,iBt)=HT.SigmaTransform(tPrm(HT.PrmInd==4,:)); %allow for either standard deviation or variance
			%we are expecting HT.Sig to be the standard deviation in any calculation.
        ...
        function p0=startingSolution(HT,X,Y,A,b0)
            ...
            if HT.NonStat(4)%TODO for either the variance and std
                v0=nan(HT.nPrm(4),1);
                for i = 1:length(v0)
                    v0(i)=HT.VarStartSol(R(B(:,i)==1)).*0.1;%.*0.1;%0.1;%
                end
            else
                v0=(ones(HT.nPrm(4),1).*HT.VarStartSol(R));
            end
            p0=[a0;b0;m0;v0];
    ...
    methods(Static)
        ...
        function G=mygradfmincon(X,Y,A,p,L)
            ...
            Alp = Alp(A,:); %expand alpha for all bins
            Kappa=sqrt(gamma(1./HT.Delta)./gamma(3./HT.Delta));
            S_Dat=sqrt(V);
            Xb=X.^Bet;  %X^b
