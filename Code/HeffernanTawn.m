classdef (Abstract) HeffernanTawn
    ...
    methods (Abstract)
        [G,H]=Gradient(obj,iP,X,Y,A,p,L)       %Gradient function overloaded
    end
    ...
    methods
        function obj=HeffernanTawn(Mrg,HTNEP,NonStationary,CV,SmpLclRsdOn)
            ...
            for i = 2:obj.nDmn % loop over margins
                ...
                %       obj.NonEmptyBins=unique([obj.NonEmptyBins;Mrg(i).NonEmptyBins;Mrg(j).NonEmptyBins]);
            end
        ...
        function obj=Fit(obj,Mrg)
            ...
            for iBt=1:obj.nBoot           %loop over bootstrap resamples
                ...
                %obj.Thr(iBt)=quantile( obj.X(:,iBt),obj.NEP(iBt)); %find the H&T threshold
        ...
        function Sml=SimulateIS(obj,Mrg,nRls)
            %Sml=SimulateIS2(obj,Mrg,nRls)
            %simulatemiportance sampling in X MC in Y|X
            % Simulatation modifed to force same reponses for all bins nRls
            % out is nRls*nBin
            %INPUTS:
            ...
            %Sml.A=ones(nRls.*obj.nBin,1);  %bin allocation
            ...
            %Pre 20221111BndStnMrg=[min(obj.Thr(:)),Mrg(1).INV_Standard(1-1e-7)];
        ...
        function Sml=SimulateMC(obj,Mrg,smlMethod,nRls,A,nA,RVX)
            ...
            tX=Sml.X; %(nA*nRls*nRtr) x nCvr copied to allow omni cat
            %% simulate on standard scale
            % [nA nRls 1 nRtr] into something that [nAxnRlsx1xnRtr by 1]
            tStnMrg=obj.SampleYgX_Stn(INV_Standard(Mrg(1),tUnf(:,1)),tI,tA); % [nAxnRlsxnRtr by nAsc]
            %% Transform to uniform
            ...
            Sml.StnMrg=permute(reshape(tStnMrg,[nA,nRls,nRtr,obj.nDmn]),[1,2,4,3]); % [nA x NRls x nAsc x nRtr]
        ...
        function Plot(obj,Mrg)
            ...
            for iDmn = 2:obj.nDmn
                ...
                % ylim([Mrg(2).INV_Standard(0.01),max(obj.RVSml.StnMrg(:,2))])
                % xlim([Mrg(1).INV_Standard(0.01),max(obj.RVSml.StnMrg(:,1))])
                hold on
                plot([min(obj.Thr),max(obj.Thr)].*[1,1]',ylim,'k--')
                plot(xlim,xlim,'k--');
            ...
            if any(obj.NonStat)
                ...
                title(sprintf('obj cross-validation lack of fit'))
            ...
            for iDmn=2:obj.nDmn %loop over associated variables
                for iRtr=1:Mrg(1).nRtr
                    ...
                    xlim([quantile(obj.RVSml.Org(end,:,iDmn,iRtr),0.01),quantile(obj.RVSml.Org(end,:,iDmn,iRtr),0.99)])
                    
                    ylabel('Cumulative Probability')
                    xlabel(sprintf('%s | %s',Mrg(iDmn).RspLbl, Mrg(1).RspLbl))
                    grid on
                    
                    if (obj.nBin<16  && obj.nBin >1 ) && (c == (obj.nDmn-1)*Mrg(1).nRtr)
    ...
    methods(Access = protected)
        ...
        function p=fit_newtonraphson(obj,X,Y,A,Lam)
            ...
            for iIt=1:maxIter %iteration loop
                ...
                for iP=1:2
                    ...
                    [G,H]=obj.Gradient(iP,X,Y,A,p,Lam);
        ...
        function p0=startingSolution(obj,X,Y,A,b0,Lam)
            ...
            p0=obj.updateStartingSolution(X,Y,A,b0,Lam);
        ...
        function [Alp,Bet,M,S]=p2Prm(obj,p)
            ...
            if obj.TwoParamFlag
                ...
                S=p(obj.PrmInd==4,:); % 1 x nD-1
            end
            
            S=obj.SigmaTransform(S); %make sure always looking at standard deviation
        end
            ...
            if obj.nBin>1
                ...
                for iP=1:4
                    if obj.nPrm(iP)>1
                        switch iP
                            case 1
                                PLOGL=PLOGL+L.*sum(tAlp(:).^2);
                            case 2
                                PLOGL=PLOGL+L.*sum(tBet(:).^2);
                            case 3
                                PLOGL=PLOGL+L.*sum(M(:).^2);
                            case 4
                                PLOGL=PLOGL+L.*sum(S(:).^2);
                        end
                    end
                end
            else
        ...
        function p=updateStartingSolution(obj,X,Y,A,b0,Lam)
            ...
            for iAsc=1:obj.nAsc
                ...
                if obj.NonStat(4)%TODO for either the variance and std
                    for i = 1:obj.nBin
                        ...
                        v0(i,iAsc)=obj.VarStartSol(R(B(:,i)==1,iAsc)).*0.1;
                        if v0(i,iAsc)==0 || isempty(R(B(:,i)==1,iAsc))
                            v0(i,iAsc)=obj.VarStartSol(R(:,iAsc)).*0.1;
                        end
            ...
            if obj.NonStat(4)==0
                v0=ones(obj.nPrm(4),obj.nAsc).*obj.VarStartSol(R);
            end
